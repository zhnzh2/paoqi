...
class Game:
    ...
    def _serialize_cannon(self, cannon: Cannon) -> dict[str, Any]:
        return {
            ...
            "short": cannon.short(),
            "record_text": format_cannon_for_record(cannon, self.cannon_record_style),
        }
    ...
    def _record_phase_change_event(self) -> None:
        self.add_last_action_event(
            self._make_event(
                ...
                current_player_name=player_name(self.current_player),
            )
        )
    ...
    def add_last_action_event(self, event: dict[str, Any]) -> None:
        self.last_action_events.append(event)

    def get_last_action_events(self) -> list[dict[str, Any]]:
        return [event.copy() for event in self.last_action_events]
    ...
    def export_board_state(self) -> list[list[dict[str, Any] | None]]:
        return self.get_board_snapshot()
    ...
    def get_board_snapshot(self) -> list[list[dict[str, Any] | None]]:
        ...
        for y in range(1, self.board.SIZE + 1):
            ...
            for x in range(1, self.board.SIZE + 1):
                row.append(self._serialize_piece_at(x, y))
    ...
    def get_cannon_snapshot(self, cannons: List[Cannon]) -> list[dict[str, Any]]:
        return [self._serialize_cannon(cannon) for cannon in cannons]
    ...
    def get_all_cannons_snapshot(self) -> dict[str, list[dict[str, Any]]]:
        ...
        return {
            "R": self.get_cannon_snapshot(red_cannons),
            "B": self.get_cannon_snapshot(blue_cannons),
        }
    ...
    def get_phase_snapshot(self) -> dict[str, Any]:
        return {
            ...
            "current_player_name": player_name(self.current_player),
            "turn_number": self.turn_number,
            "round_drop_player": self.round_drop_player,
            "chain_pass_count": self.chain_pass_count,
            "has_pending_muzzle_choice": self.has_pending_muzzle_choice(),
    ...
    def get_interaction_snapshot(self) -> dict[str, Any]:
        ...
        capturable = self.get_capturable_targets(self.current_player)

        return {
            "last_new_cannons": self.get_cannon_snapshot(self.last_new_cannons),
            "pending_muzzle_cannons": self.get_cannon_snapshot(self.pending_muzzle_cannons),
            "fire_cannon_pool": self.get_cannon_snapshot(self.fire_cannon_pool),
            "fireable_cannons": self.get_cannon_snapshot(fireable),
            "capturable_targets": [
                {
                    "x": x,
                    "y": y,
                    "piece": self._serialize_piece_at(x, y),
                }
            ...
            "last_change_reached": [
                ...
                for (x, y), (color, level) in self.last_change_reached.items()
            ],
        }
    ...
    def get_log_snapshot(self) -> dict[str, Any]:
        return {
            ...
            "last_fire_report_lines": self.last_fire_report_lines.copy(),
            "auto_action_messages": self.auto_action_messages.copy(),
            "last_action_events": self.get_last_action_events(),
        }

    def get_drop_legal_snapshot(self) -> dict[str, Any]:
        place_positions = self.board.legal_place_positions(self.current_player)
        upgrade_positions = self.board.legal_upgrade_positions(self.current_player)
            ...
            "upgrade_positions": [
                {
                    "x": x,
                    "y": y,
                    "to_level": self.board.get(x, y).level + 1,
                }
                for x, y in upgrade_positions
                if self.board.get(x, y) is not None
            ],
            "all_legal_moves_text": self.all_legal_moves(self.current_player),
        }
    ...
    def get_state_snapshot(self) -> dict[str, Any]:
        ...
        return {
            "phase_info": self.get_phase_snapshot(),
            "board": self.get_board_snapshot(),
            "cannons": self.get_all_cannons_snapshot(),
            "interaction": self.get_interaction_snapshot(),
            "drop_legal": self.get_drop_legal_snapshot(),
            "legal_actions": self.get_legal_actions_snapshot(),
            "logs": self.get_log_snapshot(),
            "score": {
                "R": red_score,
                "B": blue_score,
            },
            "cannon_record_style": self.cannon_record_style,
            "action_api": self.get_action_api_snapshot(),
        }

    def export_full_state(self) -> dict[str, Any]:
        return {
            "board": self.export_board_state(),
            "phase": self.phase,
            "current_player": self.current_player,
            "turn_number": self.turn_number,
            "round_drop_player": self.round_drop_player,
            ...
            "winner": self.winner,
            "cannon_record_style": self.cannon_record_style,
            "last_new_cannons": self.export_cannon_list(self.last_new_cannons),
            "pending_muzzle_cannons": self.export_cannon_list(self.pending_muzzle_cannons),
            "waiting_new_pool_cannons": self.export_cannon_list(self.waiting_new_pool_cannons),
            "fire_cannon_pool": self.export_cannon_list(self.fire_cannon_pool),
            "last_change_reached": [
                {
                    "x": x,
                    "y": y,
            ...
            "cannon_mouth_map": [
                ...
                for sig, mouth in self.cannon_mouth_map.items()
            ],
            "history": self.history.copy(),
            "debug_log": self.debug_log.copy(),
            "command_log": self.command_log.copy(),
    ...
    def import_full_state(self, data: dict[str, Any]) -> None:
        ...
        for y, row in enumerate(data["board"], start=1):
            for x, cell in enumerate(row, start=1):
                self.board.set(x, y, self._deserialize_piece_data(cell))

        # 2. 基础状态恢复
        self.phase = data["phase"]
        self.current_player = data["current_player"]
        self.turn_number = data["turn_number"]
        self.round_drop_player = data["round_drop_player"]
        self.chain_pass_count = data["chain_pass_count"]
        self.game_over = data["game_over"]
        self.winner = data["winner"]
        self.cannon_record_style = data["cannon_record_style"]

        # 3. 炮相关缓存恢复
        self.last_new_cannons = [
            self._deserialize_cannon_data(item)
        ...
        for item in data["cannon_mouth_map"]:
            ...
            self.cannon_mouth_map[restored_sig] = item["mouth"]
        ...
        # 5. 文本日志 / 事件缓存恢复
        self.history = data["history"].copy()
        self.debug_log = data["debug_log"].copy()
        self.command_log = data["command_log"].copy()
        self.last_fire_report_lines = data["last_fire_report_lines"].copy()
        self.auto_action_messages = data["auto_action_messages"].copy()
        self.last_action_events = [event.copy() for event in data["last_action_events"]]
        self.undo_stack = []

    @classmethod
    def from_exported_state(cls, data: dict[str, Any]) -> "Game":
        game = cls()
        game.import_full_state(data)
        return game

    def export_import_roundtrip_snapshot(self) -> dict[str, Any]:
        data = self.export_full_state()
        clone = Game.from_exported_state(data)
        return clone.get_state_snapshot()

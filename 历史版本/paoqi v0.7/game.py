...
class Game:
    ...
    def format_pos(self, pos: Position) -> str:
        x, y = pos
        return f"({x}, {y})"
    ...
    def mouth_text(self, mouth: str) -> str:
        mapping = {
            "L": "左",
            "R": "右",
            "U": "上",
    ...
    def cannon_direction_text(self, direction: str) -> str:
        mapping = {
            "H": "横",
            "V": "纵",
        }
    ...
    def format_cannon_positions_for_record(self, cannon: Cannon) -> str:
        pos_texts = [self.format_pos(pos) for pos in cannon.positions]
        return "[" + ", ".join(pos_texts) + "]"
    
    def format_cannon_tuple_record(self, cannon: Cannon) -> str:
        k = cannon.level
        n = len(cannon.positions)
        a_text = self.format_cannon_positions_for_record(cannon)
        dir_text = self.cannon_direction_text(cannon.direction)

        if cannon.mouth is None:
            mouth_text = "未定口"
        else:
            mouth_text = self.mouth_text(cannon.mouth)

        return f"({k}, {n}, {a_text}, {dir_text}, {mouth_text})"

    def copy_board_grid(self) -> list[list[Piece | None]]:
    ...
    def copy_cannon(self, cannon: Cannon) -> Cannon:
        copied = Cannon(
            color=cannon.color,
            level=cannon.level,
    ...
    def copy_cannon_list(self, cannons: List[Cannon]) -> List[Cannon]:
        return [self.copy_cannon(c) for c in cannons]
    
    def snapshot_state(self) -> dict:
        return {
            "board_grid": self.copy_board_grid(),
            ...
            "winner": self.winner,
            "last_new_cannons": self.copy_cannon_list(self.last_new_cannons),
            "pending_muzzle_cannons": self.copy_cannon_list(self.pending_muzzle_cannons),
            "last_fire_report_lines": self.last_fire_report_lines.copy(),
            "last_change_reached": self.last_change_reached.copy(),
            "cannon_mouth_map": self.cannon_mouth_map.copy(),
            "fire_cannon_pool": self.copy_cannon_list(self.fire_cannon_pool),
            "waiting_new_pool_cannons": self.copy_cannon_list(self.waiting_new_pool_cannons),
            "phase": self.phase,
    ...
    def restore_state(self, snapshot: dict) -> None:
        self.board.grid = snapshot["board_grid"]
        self.current_player = snapshot["current_player"]
        self.turn_number = snapshot["turn_number"]
        self.history = snapshot["history"]
        self.debug_log = snapshot["debug_log"]
        self.game_over = snapshot["game_over"]
        self.winner = snapshot["winner"]
        self.last_new_cannons = snapshot["last_new_cannons"]
        self.pending_muzzle_cannons = snapshot["pending_muzzle_cannons"]
        self.last_fire_report_lines = snapshot["last_fire_report_lines"]
        self.last_change_reached = snapshot["last_change_reached"]
        self.cannon_mouth_map = snapshot["cannon_mouth_map"]
        self.fire_cannon_pool = snapshot["fire_cannon_pool"]
        self.waiting_new_pool_cannons = snapshot["waiting_new_pool_cannons"]
        self.phase = snapshot["phase"]
        self.round_drop_player = snapshot["round_drop_player"]
        self.chain_pass_count = snapshot["chain_pass_count"]
        self.cannon_record_style = snapshot["cannon_record_style"]
    ...
    def format_cannon_for_record(self, cannon: Cannon) -> str:
        if self.cannon_record_style == 1:
            return cannon.short()

        if self.cannon_record_style == 2:
            return self.format_cannon_tuple_record(cannon)

        # 兜底：如果配置值异常，默认仍使用当前简洁形式
        return cannon.short()

    def format_cannon_with_mouth_for_record(self, cannon: Cannon) -> str:
        base = self.format_cannon_for_record(cannon)

        if cannon.mouth is None:
            return base

        # 模式2中，五元组本身已经把炮口方向写进去了
        if self.cannon_record_style == 2:
            return base

        return f"{base}（{self.mouth_text(cannon.mouth)}口）"
    ...
    def record_new_cannons(
        ...
        for cannon in cannons:
            ...
            if manual_signature_to_mouth is not None and sig in manual_signature_to_mouth:
                ...
                copied = self.copy_cannon(cannon)
                copied.mouth = mouth
                texts.append(self.format_cannon_with_mouth_for_record(copied))
            else:
                texts.append(self.format_cannon_for_record(cannon))

        player = self.player_name(cannons[0].color)
    ...
    def record_fire(self, cannon: Cannon) -> None:
        self.record(
            f"{self.player_name(cannon.color)}打了{self.format_cannon_for_record(cannon)}"
        )

    def record_capture(self, pos: Position) -> None:
        self.record(
            f"{self.player_name(self.current_player)}吃了{self.format_pos(pos)}处棋子"
        )
    ...
    def debug_text(self) -> str:
        ...
        for i, item in enumerate(self.debug_log, start=1):
            lines.append(f"  {i}. {item}")
    ...
    def apply_place(self, x: int, y: int) -> None:
        ...
        self.record(f"{self.player_name(color)}在({x}, {y})落子")
    ...
    def set_cannon_mouth(self, index: int, direction_text: str) -> None:
        ...
        self.record_new_cannons(
            [cannon],
            manual_signature_to_mouth={self.cannon_signature(cannon): mouth},
        )
